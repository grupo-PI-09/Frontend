...
export function SideBar({ collapsed, setCollapsed }) {
    ...
    useEffect(() => {
        function sincronizarUsuario() {
            setUsuarioAtual(getUsuario())
        }

        const nomeEvento = getAuthUpdatedEventName()
        window.addEventListener(nomeEvento, sincronizarUsuario)
        window.addEventListener('storage', sincronizarUsuario)

        return () => {
            window.removeEventListener(nomeEvento, sincronizarUsuario)
            window.removeEventListener('storage', sincronizarUsuario)
        }
    }, [])

    const nomeUsuario = usuarioAtual?.nome || 'L'
            ...
            <div className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
                ...
                <nav className="menu">
                    ...
                    <div
                        ...
                        title="Notificações">
                        <img src={imgNotif} className="menu-icon" />
